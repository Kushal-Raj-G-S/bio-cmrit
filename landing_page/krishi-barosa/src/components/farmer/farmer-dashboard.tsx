...
export function FarmerDashboard(): React.JSX.Element {
  ...
  const recordStageImagesOnBlockchain = async (
    stageName: string,
    imageUrls: string[],
    firstImagePending: boolean
  ): Promise<boolean> => {
    if (!selectedBatch || !user?.id) return firstImagePending;

    let pending = firstImagePending;
    ...
    for (const imageUrl of imageUrls) {
      const response = await fetch('/api/blockchain/record-image', {
        ...
        body: JSON.stringify({
          ...
          stageName,
          imageUrl,
          isFirstImage: pending,
          farmerDetails: {
            name: user.name,
            location: typeof user.location === 'string' ? user.location : 'Unknown'
          },
          batchDetails: {
            cropType: selectedBatch.name || selectedBatch.category || 'Unknown',
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || `Blockchain record failed for stage "${stageName}"`);
      }

      // Only the first successful image across the whole batch should be marked first.
      pending = false;
    }
  ...
  const handleSubmitStage = async (stageIndex: number) => {
    ...
    try {
      ...
      // Record each uploaded image to blockchain via frontend bridge.
      let firstImagePending = stages.length === 0;
      firstImagePending = await recordStageImagesOnBlockchain(stage.name, uploadedImageUrls, firstImagePending);
  ...
  const handleSubmitAllStages = async () => {
    ...
    try {
      ...
      // Record all uploaded stage images to blockchain via frontend bridge.
      let firstImagePending = stages.length === 0;
      for (const stage of stagesWithImages) {
        firstImagePending = await recordStageImagesOnBlockchain(stage.name, stage.images, firstImagePending);
      }
  ...
  const handleDownloadCertificate = async (batch: Batch) => {
    try {
      if (!batch.qrCode) {
        toast.error('QR code is not available for this batch yet');
        return;
      }

      toast.info('Opening QR code...');
      window.open(batch.qrCode, '_blank', 'noopener,noreferrer');
      
      toast.success('QR opened successfully');
    } catch (error) {
                ...
                <div key={batch.id || index} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="ml-4 flex gap-2">
                      ...
                      {batch.verificationStatus === 'VERIFIED' && batch.qrCode && (
                        <Button
                          variant="outline"
                          size="sm"
        ...
        <DialogContent className="sm:max-w-3xl bg-white rounded-2xl border-0 shadow-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedBatch && (
            <div className="space-y-6 py-4">
              ...
              {selectedBatch.verificationStatus === 'VERIFIED' && selectedBatch.qrCode && (
                <div className="border-t border-slate-200 pt-6 mt-6">
                  <div className="bg-gradient-to-r from-amber-50 via-yellow-50 to-amber-50 rounded-xl p-6 border-2 border-amber-200">
                    <div className="flex items-center justify-between gap-6">
                      ...
                      <div className="bg-white p-2 rounded-lg border border-amber-300 shadow-sm">
                        <img
