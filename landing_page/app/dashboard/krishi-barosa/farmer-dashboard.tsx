...
const KB_API_BASE = '/api/krishi-barosa';
...
export function FarmerDashboard(): React.JSX.Element {
  ...
  const [kbFarmerId, setKbFarmerId] = useState<string | null>(null);
  ...
  const getActiveFarmerId = () => kbFarmerId || batches[0]?.farmerId || user?.id || null;

  // KrishiBarosa keeps its own user ids; prefer that id for CRUD to satisfy FK constraints.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      const raw = localStorage.getItem('krishibarosa-user');
      if (!raw) return;
      const parsed = JSON.parse(raw);
      if (parsed?.id) {
        setKbFarmerId(parsed.id);
      }
    } catch {
  ...
  const fetchStages = async (batchId: string) => {
    try {
      ...
      const response = await fetch(`${KB_API_BASE}/stages?batchId=${batchId}`);
      if (response.ok) {
  ...
  const uploadFilesToSupabase = async (files: File[], stageName: string, stageId?: string): Promise<string[]> => {
    const activeFarmerId = getActiveFarmerId();
    const uploadPromises = files.map(async (file) => {
      ...
      if (activeFarmerId) {
        formData.append('farmerId', activeFarmerId);
      }

      const response = await fetch(`${KB_API_BASE}/upload`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleSubmitStage = async (stageIndex: number) => {
    const activeFarmerId = getActiveFarmerId();
    if (!selectedBatch || !activeFarmerId) return;
    ...
    try {
      ...
      const response = await fetch(`${KB_API_BASE}/stages`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      const activeFarmerId = kbFarmerId || user?.id;
      if (!activeFarmerId) return;
      
      try {
        setIsLoading(true);
        const response = await fetch(`${KB_API_BASE}/batches?farmerId=${activeFarmerId}`);
        if (response.ok) {
          const data = await response.json();
          setBatches(data);
          if (!kbFarmerId && Array.isArray(data) && data.length > 0 && data[0]?.farmerId) {
            setKbFarmerId(data[0].farmerId);
          }
        }
      } catch (error) {
  ...
  const handleCreateBatch = async (): Promise<void> => {
    ...
    const activeFarmerId = kbFarmerId || user?.id || batches[0]?.farmerId;
    if (!activeFarmerId) {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`${KB_API_BASE}/batches`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
